...
export default function ResultsReportModule() {
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      alert('CSV export failed: ' + (err.response?.data?.error || err.message));
    } finally {
      ...
      {loadingData ? (
        ...
        <div className="space-y-8">
          ...
          {isConcluded && (
            <div>
              {winner ? (
                <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-amber-500/10 via-indigo-500/10 to-emerald-500/10 border border-amber-500/20 flex flex-col sm:flex-row items-center justify-between gap-6">
